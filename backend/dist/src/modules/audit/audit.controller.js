...
let AuditController = class AuditController {
    ...
    async getAllLogs(req) {
        const user = req.user;
        if (user.role !== 'SUPER_ADMIN') {
            throw new common_1.ForbiddenException('Only SUPER_ADMIN can view all audit logs');
        }
        return this.auditService.getAllLogs();
    }
    async getDocumentLogs(id, req) {
        const user = req.user;
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], AuditController.prototype, "getDocumentLogs", null);
